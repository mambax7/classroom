<?php
// $Id: imagemanager.php,v 1.11.4.1 2004/10/13 13:07:41 mithyt2 Exp $
//  ------------------------------------------------------------------------ //
...
$nocode = isset($_REQUEST['nocode']) ? $_REQUEST['nocode'] : false;
...
if ($op == 'upload') {
	...
	$form->addElement(new XoopsFormHidden('nocode', $nocode));
...
if ($op == 'doupload') {
	...
	header('location: imagemanager.php?cat_id='.$imgcat_id.'&target='.$target.'&nocode='.$nocode);
}
