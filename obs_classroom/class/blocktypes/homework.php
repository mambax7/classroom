...
class HomeworkBlock extends ClassroomBlock {

    var $datetable;
    
    function HomeworkBlock() {
        $this->ClassroomBlock();
        $this->datetable = $this->db->prefix('cr_homework');
        $this->assignVar('template', 'cr_blocktype_homework.html');
        ...
        if (count($entries) > 0) {
            ...
            foreach ($entries as $id => $entry) {
                ...
                $currententries .= "<tr class='".$class."'><td>".formatTimestamp($entry['assigned'], 's', "0")."
                                    <td>".$myts->htmlSpecialChars($entry['value'])."</td>
    ...
    */
    function buildBlock() {
        ...
        foreach ($entries as $key => $entry) {
            $block['entries'][$key]['value'] = $myts->htmlSpecialChars($entry['value']);
    ...
    */
    function updateBlock() {
        ...
        if (isset($_POST['fieldid']) && $_POST['fieldid'] > 0) {
            if ($_POST['text'] != "") {
                $sql = "UPDATE ".$this->table." SET value='".$myts->addSlashes($_POST['text'])."' WHERE blockid=".$this->getVar('blockid')." AND fieldid=".intval($_POST['fieldid']);
                $this->db->query($sql);
                $sql = "UPDATE ".$this->datetable." SET assigned=".strtotime($_POST['assigned']).", due=".strtotime($_POST['due'])." WHERE fieldid=".intval($_POST['fieldid']);
                return $this->db->query($sql);
            }
        }
        else {
            if ($_POST['text'] != "") {
                $sql = "INSERT INTO ".$this->table." (blockid, value, weight) VALUES (".intval($this->getVar('blockid')).", '".$myts->addSlashes($_POST['text'])."', 0)";
                $this->db->query($sql);
