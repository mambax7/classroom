...
class QuoteBlock extends ClassroomBlock {
    
    function QuoteBlock() {
        $this->ClassroomBlock();
        $this->assignVar('template', 'cr_blocktype_quote.html');
    ...
    */
    function buildForm() {
        ...
        if (isset($_GET['quoteid'])) {
            ...
            $quote = array('quote' => '', 'author' => '', 'weight' => 0);
        }
        
        $form = new XoopsThemeForm('', 'quoteform', 'manage.php');
        $form->addElement(new XoopsFormText(_CR_MA_QUOTE, 'quote', 50, 255, $myts->htmlSpecialChars($quote['quote'])), true);
        $form->addElement(new XoopsFormText(_CR_MA_AUTHOR, 'author', 40, 100, $myts->htmlSpecialChars($quote['author'])), true);
        ...
        if (isset($_POST['quoteid']) && $_POST['quoteid'] > 0) {
            $sql = "UPDATE ".$this->table." 
                    SET value='".$myts->addSlashes($_POST['quote']).";".$myts->addSlashes($_POST['author'])."',
                        weight=0,
                        updated=".time()."
                    WHERE blockid=".$this->getVar('blockid')." AND fieldid=".intval($_POST['quoteid']);
        }
        else {
    ...
    */
    function getQuotes() {
        ...
        while ($row = $this->db->fetchArray($result)) {
            ...
            $row['quote'] = $values[0];
            $row['author'] = $values[1];
            $ret[$row['fieldid']] = array('quoteid' => $row['fieldid'], 
                                         'quote' => $myts->htmlSpecialChars($row['quote']),
                                         'author' => $myts->htmlSpecialChars($row['author']),
                                         'weight' => $row['weight']);
            unset($values);
        }
        return $ret;
    ...
    */
    function getRandomQuote() {
        ...
        $values = explode(';', $row['value']);
        $row['quote'] = $myts->htmlSpecialChars($values[0]);
        $row['author'] = $myts->htmlSpecialChars($values[1]);
