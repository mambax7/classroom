...
$sform->addElement(new XoopsFormText(_CR_MA_WEIGHT, 'weight', 10, 10, intval($edit_school->getVar('weight'))));

$membership_handler =& xoops_gethandler('membership');
$criteria = new Criteria('groupid', "(".implode(',',$xoopsModuleConfig['head_group']).")", 'IN');
$allmemberships = $membership_handler->getObjects($criteria);
unset($criteria);
$userids = array();
...
$sform->addElement(new XoopsFormDhtmlTextArea(_CR_MA_DESCRIPTION, 'description', $myts->htmlSpecialChars($edit_school->getVar('description', 'n')), 35, 50));
...
if ($edit_school->getVar('schoolid') > 0) {
    ...
    $moderators = $gperm_handler->getGroupIds('school', $edit_school->getVar('schoolid'), $xoopsModule->getVar('mid'));
}
